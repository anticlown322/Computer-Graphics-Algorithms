...
public class SceneManager
{
    ...
    public void TransformObject()
    {
        ...
        var view = Transformations.CreateViewMatrix(CameraModel.EyePosition, CameraModel.TargetPosition, CameraModel.UpVector);
        
        var projection = Transformations.CreateProjectionMatrix(CameraModel.Fov, CameraModel.AspectRatio, CameraModel.ZNear, CameraModel.ZFar);
        
        var viewport = Transformations.CreateViewportMatrix(CanvasWidth, CanvasHeight, 0.0f, 0.0f);

        var world = Transformations.CreateTransformMatrix(ObjectModel.Position, ObjectModel.Rotation, ObjectModel.Scale);
